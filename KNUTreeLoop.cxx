...
void KNUTree::Loop()
{/*{{{*/
  KNUOUT << "Begin to loop over " << nEvents << " events." << endl;
  KNUTree::InitLoop();
...
/**
 * @brief Loop over events. This method allows multiple particle event.
 * @return no return.
 */
...
{/*{{{*/
  ...
  for( unsigned long int e = 0; e < nEvents; e++ )
  {
    ...
    for( int iParticle = 0; iParticle < nParticle; iParticle++ )
    {
      ...
      if( !pParticle ) { hGoodParticleCounter->Fill(1); continue; }
      pBetaH = pParticle->pBetaH();
      if( !pBetaH ) { hGoodParticleCounter->Fill(2); continue; }
      pTrTrack = pParticle->pTrTrack();
      if( !pTrTrack ) { hGoodParticleCounter->Fill(3); continue; }
      ...
      trkFitCodeL1Inner              = mfit;
      if( trkFitCodeL1Inner < 0 ) continue;
      trkRigidityL1Inner             = rigidity;
      trkRigidityInverseErrorL1Inner = pTrTrack->GetErrRinv(trkFitCodeL1Inner);
      trkReducedChisquareL1InnerX    = pTrTrack->GetNormChisqX(trkFitCodeL1Inner);
      trkReducedChisquareL1InnerY    = pTrTrack->GetNormChisqY(trkFitCodeL1Inner);
