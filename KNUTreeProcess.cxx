 ...
 * @return
 */
void KNUTree::Process()
{
  KNUOUT << "Begin to loop over " << nEvents << " events. [ISS Mode]" << endl;
  ...
  for( unsigned long int e = 0; e < nEvents; e++ )
  {
    ...
    hEvtCounter->Fill(0);
    if( !IsHardwareStatusGood(pAMSEvent) ) continue;
    hEvtCounter->Fill(1);
    if( IsUnbiasedPhysicsTriggerEvent(pAMSEvent) ) continue;
    hEvtCounter->Fill(2);
    if( pAMSEvent->IsInSAA() ) continue; // Reject this event if the event is recorded in South Atlantic Anomaly region.
    AMSSetupR::RTI rti;
